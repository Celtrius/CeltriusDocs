// node_modules/.pnpm/@royalfig+share-button@1.0.26/node_modules/@royalfig/share-button/dist/share-button.es.js
...
var M = ".wrapper{--color-button-text: hsl(240deg 10% 3.9%);--color-button-bg: hsl(0deg 0% 98%);--color-copy-success: hsl(120deg 99% 43%);--color-popover-bg: hsl(240deg 10% 3.9%);--color-popover-bg-accent: hsl(240deg 3.7% 15.9%);--color-popover-text: hsl(0deg 0% 98%);--color-popover-border: hsl(240deg 3.7% 15.9%)}";
function A(i) {
...
function I({
  ...
  function s(x, l, g) {
    ...
    return a.classList.add("social-media", l.toLowerCase()), a.href = encodeURI(n), a.setAttribute("aria-label", `Share on ${l}`), a.setAttribute("target", "_blank"), a.setAttribute("rel", "noopener noreferrer"), a.setAttribute("part", `share-link ${l.toLowerCase()}`), a.innerHTML = `${x} ${t ? "" : ` ${l}`}`, a;
  }
...
var k = class extends HTMLElement {
  ...
  render() {
    ...
    u.append(v, x), this.shadow.replaceChildren(u);
    let l = null;
    if (!d && v && v.addEventListener("click", (a) => {
      ...
      if (this.isPopoverSupport) {
        ...
        let f = `${c.left + c.width / 2 - l.width / 2}px`;
        c.left < 100 && (f = `${c.left + c.width / 2 - l.width * 0.25}px`, r.classList.add("left-adjust")), c.right > window.innerWidth - 100 && (f = `${c.left + c.width / 2 - l.width * 0.75}px`, r.classList.add("right-adjust"));
        const y = window.scrollY;
        r.style.left = f, document.documentElement.clientHeight / 2 > c.y ? (r.style.top = `${y + c.top + c.height}px`, r.classList.remove("down"), r.classList.add("up")) : (r.style.top = `${y + c.top - l.height}px`, r.classList.remove("up"), r.classList.add("down"));
        return;
      }
